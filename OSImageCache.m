...
#import "OSImageCache.h"
#import "NSData+VPBase64.h"

static inline NSString *VPImageCacheDirectory() {
	static NSString *_VPImageCacheDirectory;
	static dispatch_once_t onceToken;
    
	dispatch_once(&onceToken, ^{
		_VPImageCacheDirectory = [[NSHomeDirectory() stringByAppendingPathComponent:@"Library/Caches/OSCache"] copy];
	});

	return _VPImageCacheDirectory;
}
...
static inline NSString *cachePathForKey(NSString *key) {
    NSString *fileName = [NSString stringWithFormat:@"OSImageCache-%lu", (unsigned long)[key hash]];
	return [VPImageCacheDirectory() stringByAppendingPathComponent:fileName];
}

@interface OSImageCache ()
...
@implementation OSImageCache

@synthesize diskOperationQueue = _diskOperationQueue;

+ (OSImageCache *) sharedCache {
	static OSImageCache *_sharedCache = nil;
	static dispatch_once_t onceToken;

	dispatch_once(&onceToken, ^{
		_sharedCache = [[OSImageCache alloc] init];
	});
...
- (id) init {
    ...
    [[NSFileManager defaultManager] createDirectoryAtPath:VPImageCacheDirectory()
...
- (void) _downloadAndWriteImageForURL:(NSURL *)url key:(NSString *)key completionBlock:(void (^)(UIImage *image))completion failureBlock:(void (^)(NSURLRequest *request, NSURLResponse *response, NSError* error))failure
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (!i)
        {
            ...
            NSError* error = [NSError errorWithDomain:@"OSImageCacheErrorDomain" code:1 userInfo:errorDetail];
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void) removeAllObjects {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSError *error = nil;
        NSArray *directoryContents = [fileMgr contentsOfDirectoryAtPath:VPImageCacheDirectory() error:&error];

        if (error == nil) {
            for (NSString *path in directoryContents) {
                NSString *fullPath = [VPImageCacheDirectory() stringByAppendingPathComponent:path];
...
- (UIImage *) imageForURL:(NSURL *)url key:(NSString*)key delegate:(id<OSImageCacheDelegate>)d {
	if(!url) return nil;
...
- (UIImage *) imageForURL:(NSURL *)url delegate:(id<OSImageCacheDelegate>)d {
