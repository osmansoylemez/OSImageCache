...
#import "UIImageView+OSImageCache.h"
#import "OSImageCache.h"
#import <objc/runtime.h>

static char kOSImageURLObjectKey;

@interface UIImageView (_OSImageCache)

@property (readwrite, nonatomic, retain, setter = vp_setImageURL:) NSURL *vp_imageURL;

@end

@implementation UIImageView (_OSImageCache)

@dynamic vp_imageURL;

@end

@implementation UIImageView (OSImageCache)

#pragma mark - Private Setters

- (NSURL *) jm_imageURL {
    return (NSURL *)objc_getAssociatedObject(self, &kOSImageURLObjectKey);
}
- (void) jm_setImageURL:(NSURL *)imageURL {
    objc_setAssociatedObject(self, &kOSImageURLObjectKey, imageURL, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void) setImageWithURL:(NSURL *)url key:(NSString*)key placeholder:(UIImage *)placeholderImage completionBlock:(void (^)(UIImage *image))completionBlock failureBlock:(void (^)(NSURLRequest *request, NSURLResponse *response, NSError* error))failureBlock{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (key) {
            i = [[OSImageCache sharedCache] cachedImageForKey:key];
        } else {
            i = [[OSImageCache sharedCache] cachedImageForURL:url];
        }

        if(i) {
            dispatch_async(dispatch_get_main_queue(), ^{
                safeSelf.vp_imageURL = nil;
            ...
            [[OSImageCache sharedCache] imageForURL:url key:key completionBlock:^(UIImage *image) {
